...
def set_warn_mode(update, context):
    ...
    if args:
        if args[0].lower() in ("kick", "soft"):
            ...
            if conn:
                text = (update.effective_message, "Too many warnings now will result in a kick on *{}*! Users will be able to join again.").format(chat_name)
            else:
            ...
            send_message(update.effective_message, tl(update.effective_message, "I only understood kick/ban/mute!"))
    else:
        limit, soft_warn, warn_mode = sql.get_warn_setting(chat.id)
        if not soft_warn:
            if not warn_mode:
                if conn:
                    text = (update.effective_message, "The current warning is set to *kick* when the user exceeds the limit on *{}*.").format(chat_name)
                else:
                ...
                if conn:
                    text = (update.effective_message, "The current warning is set to *ban* user when exceeding the limit at *{}*.").format(chat_name)
                else:
                    text = (update.effective_message, "The current warning is set to *ban* user when exceeding limits.")
            elif warn_mode == 3:
                if conn:
            ...
            if conn:
                text = (update.effective_message, "The warning is currently set to *ban* users when they exceed the limit on *{}*.").format(chat_name)
            else:
                text = (update.effective_message, "Warning is currently set for *ban* users when exceeding limits.")
            send_message(update.effective_message, text,
                           parse_mode=ParseMode.MARKDOWN)
    return ""
